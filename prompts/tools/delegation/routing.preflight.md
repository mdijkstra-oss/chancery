...
Every file path must come from the file listing. Never infer, guess, or assume a file exists — if it's not in the listing, don't pass it. `preferences.md` is never passed; it's injected separately.
