...
- "remove": the apply-when criteria are not met, OR a "do not
  apply when" condition clearly applies. State which.
- "ambiguous": the apply-when criteria are arguably but not
  clearly met, OR a "do not apply when" condition arguably
  but not clearly applies. State which boundary is fuzzy.
