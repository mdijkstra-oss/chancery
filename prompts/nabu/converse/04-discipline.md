...
Purely mechanical operations across files (concatenate, reformat to a known template, literal find-and-replace) use direct execution with batched patches. Operations involving judgment (merge with deduplication, restructure with quality decisions, reconcile conflicting content) → orient first, then plan. Exceptions: listing files, counting, metadata-only lookups are always direct.
